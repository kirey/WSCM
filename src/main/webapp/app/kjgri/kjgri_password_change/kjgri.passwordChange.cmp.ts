...
import { Captcha, Alert } from '../../kjcore/shared/models';
...
export class PasswordChangeCmp implements OnInit {
    ...
    forgotPasswordRequest(email: string): void {
        this.subscriptions['forgotPasswordRequest'] = this._passwordChangeService.forgotPasswordRequestRest(email).subscribe(
            (res: RestResponse<any>) => {
