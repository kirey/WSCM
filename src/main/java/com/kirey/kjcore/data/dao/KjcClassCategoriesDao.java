...
public class KjcClassCategoriesDao extends KjcBaseDao{
	...
	/**A method used for extracting category with validation class from database
	 * @return KjcClassCategories with validation
	 */
	@Transactional(readOnly = true)
	public KjcClassCategories getValidationCategory(){
		return (KjcClassCategories) sessionFactory.getCurrentSession().createCriteria(KjcClassCategories.class)
				.add(Restrictions.eq("name", AppConstants.REPORT_VALIDATION_CLASS))
				.uniqueResult();
	}
