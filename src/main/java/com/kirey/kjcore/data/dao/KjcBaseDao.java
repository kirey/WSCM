...
public class KjcBaseDao {
	...
	/**A method used for extracting all objects with example added
	 * @param instance
	 * @return List<Object> containing all objects with example added
	 */
	@Transactional(readOnly = true)
	public <T> List<T> findByExample(T instance) {
		List<T> results = sessionFactory.getCurrentSession().createCriteria(entityClass).add(create(instance)).list();
		return results;
	}
